...
router.get('/', (req, res) => {
    ...
    const { status, busca, data_inicio, data_fim, vendedor } = req.query;
    
    let sql = `
         SELECT 
            p.id, p.ordem_compra_id, p.descricao, p.valor, p.data_vencimento, 
            p.status as status_db, p.data_pagamento, p.metodo_pagamento, p.baixa_automatica,
    ...
    if (vendedor && vendedor !== 'todos') {
        whereClauses.push("(oc.vendedor_id = ? OR p.vendedor_id = ?)");
...
// **NOVA ROTA: Atualizar valor da parcela**
router.patch('/valor/:id', (req, res) => {
    const { id } = req.params;
...
// **NOVA ROTA: Atualizar tipo de pagamento da parcela**
router.patch('/tipo-pagamento/:id', (req, res) => {
    const { id } = req.params;
...
// **NOVA ROTA: Ativar/Desativar baixa automática**
router.patch('/baixa-automatica/:id', (req, res) => {
    const { id } = req.params;
    ...
    if (typeof ativar !== 'boolean') {
        return res.status(400).json({ 
            success: false, 
            error: 'Parâmetro ativar deve ser true ou false' 
        });
    }
    
    const sql = `UPDATE Parcelas SET baixa_automatica = ? WHERE id = ? AND status = 'Pendente'`;
    
    db.run(sql, [ativar ? 1 : 0, id], function(err) {
        if (err) {
            console.error('Erro ao configurar baixa automática:', err);
            return res.status(500).json({ 
                success: false, 
                error: 'Erro interno do servidor' 
            });
        ...
        res.json({
            success: true,
            message: ativar ? 'Baixa automática ativada com sucesso' : 'Baixa automática desativada com sucesso'
        });
    });
});
